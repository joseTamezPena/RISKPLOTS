...
data <- as.data.frame(model.matrix(status~.*.,data,na.action=NULL))
data$`(Intercept)` <- NULL
colnames(data) <-str_replace_all(colnames(data),":","_")
colnames(data) <-str_replace_all(colnames(data),"\\.","_")

dataLung <- cbind(time=time/365,status=status,data)
...
topvar <- univariate_BinEnsemble(dataLung,"status",pvalue=0.05)
pander::pander(topvar)
topv <- min(5,length(topvar))
topFive <- names(topvar)[1:topv]
for (topf in topFive)
{
  tmpRR <- RRPlot(cbind(dataLung$status,dataLung[,topf]),title=topf)
}

```

## Exploring Survival of Features with RRPlot

```{r results = "asis", warning = FALSE , dpi=300, fig.height= 5.0, fig.width= 7.0}
for (topf in topFive)
{
  tmpRR <- RRPlot(cbind(dataLung$status,dataLung[,topf]),
...
timeinterval <- 2*mean(subset(dataLung,status==1)$time)

h0 <- sum(dataLung$status & dataLung$time <= timeinterval)
h0 <- h0/sum((dataLung$time > timeinterval) | (dataLung$status==1))
